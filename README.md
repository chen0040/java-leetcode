# java-leetcode

My daily LeetCode solutions

The solutions were written while I was solving these problems during practice, therefore while the solutions were accepted in LeetCode on Submission, they may not represent the best solution and the source 
codes may not be optimized

### Day 1 - Easy

* [Array Partition I](https://leetcode.com/problems/array-partition-i/tabs/description)
* [Big Countries](https://leetcode.com/problems/big-countries/tabs/description)
* [Distribute Candies](https://leetcode.com/problems/distribute-candies/tabs/description)
* [Hamming Distance](https://leetcode.com/problems/hamming-distance/tabs/description)
* [Merge Two Binary Trees](https://leetcode.com/problems/merge-two-binary-trees/tabs/description)
* [Number Complement](https://leetcode.com/problems/number-complement/tabs/description)
* [Reshape the Matrix](https://leetcode.com/problems/reshape-the-matrix/tabs/description)
* [Reverse String](https://leetcode.com/problems/reverse-string/tabs/description)
* [Reverse Words in String III](https://leetcode.com/problems/reverse-words-in-string-iii/description/)
* [Swap Salary](https://leetcode.com/problems/swap-salary/tabs/description)

### Day 1 - Medium

* [Battleships in a A Board](https://leetcode.com/problems/battleships-in-a-board/tabs/description)
* [Binary Search Tree Iterator](https://leetcode.com/problems/binary-search-tree-iterator/tabs/description)
* [Binary Tree Preorder Traversal](https://leetcode.com/problems/binary-tree-preorder-traversal/tabs/description)
* [Complex Number Multiplication](https://leetcode.com/problems/complex-number-multiplication/tabs/description)
* [Find Duplicate File in System](https://leetcode.com/problems/find-duplicate-file-in-system/description/)
* [Single Element in Sorted Array](https://leetcode.com/problems/single-element-in-a-sorted-array/description/)
* [Sort Characters by Frequency](https://leetcode.com/problems/sort-characters-by-frequency/tabs/description)
* [Spiral Matrix](https://leetcode.com/problems/spiral-matrix/description/)
* [Spiral Matrix II](https://leetcode.com/problems/spiral-matrix-ii/description/)
* [Top K Frequent Items](https://leetcode.com/problems/top-k-frequent-elements/description/)

### Day 2 - Easy

* [Average Of Levels In Binary Tree](https://leetcode.com/problems/average-of-levels-in-binary-tree/tabs/description)
* [Detect Capitals](https://leetcode.com/problems/detect-capital/tabs/description)
* [Find All Numbers Disappear in Array](https://leetcode.com/problems/find-all-numbers-disappeared-in-an-array/tabs/description)
* [FizzBuzz](https://leetcode.com/problems/fizz-buzz/tabs/description)
* [Invert Binary Tree](https://leetcode.com/problems/invert-binary-tree/tabs/description)
* [Island Parameter](https://leetcode.com/problems/island-perimeter/tabs/description)
* [Max Consecutive Ones](https://leetcode.com/problems/max-consecutive-ones/tabs/description)
* [Next Greater Element](https://leetcode.com/problems/next-greater-element-i/tabs/description)
* [Not Boring Movies](https://leetcode.com/problems/not-boring-movies/tabs/description)
* [Single Number](https://leetcode.com/problems/single-number/tabs/description)
* [Sum of Two Integers](https://leetcode.com/problems/sum-of-two-integers/tabs/description)

### Day 2 - Medium

* [Add One Row To Tree Node](https://leetcode.com/problems/add-one-row-to-tree/tabs/description)
* [Array Nesting](https://leetcode.com/problems/array-nesting/tabs/description)
* [Count Bits](https://leetcode.com/problems/counting-bits/tabs/description)
* [Find All Duplicates in Array](https://leetcode.com/problems/find-all-duplicates-in-an-array/tabs/description)
* [Find Bottom Left Tree Value](https://leetcode.com/problems/find-bottom-left-tree-value/tabs/description)
* [Friend Circles](https://leetcode.com/problems/friend-circles/tabs/description)
* [Inorder Traversal](https://leetcode.com/problems/binary-tree-inorder-traversal/tabs/description)
* [Largest Value in Each Tree Row](https://leetcode.com/problems/find-largest-value-in-each-tree-row/tabs/description)
* [Shuffle an Array](https://leetcode.com/problems/shuffle-an-array/description/)
* [Single Number III](https://leetcode.com/problems/single-number-iii/tabs/description)
* [Total Hamming Distance](https://leetcode.com/problems/total-hamming-distance/tabs/description)

### Day 3 - Easy

* [Binary Tree Tilt](https://leetcode.com/problems/binary-tree-tilt/description/)
* [Contains Duplicates](https://leetcode.com/problems/contains-duplicate/description/)
* [Excel Sheet Column Numbers](https://leetcode.com/problems/excel-sheet-column-number/description/)
* [Find Unique Characters in String](https://leetcode.com/problems/first-unique-character-in-a-string/description/)
* [Majority Element](https://leetcode.com/problems/majority-element/description/)
* [Relative Ranks](https://leetcode.com/problems/relative-ranks/description/)
* [Reverse Linked List](https://leetcode.com/problems/reverse-linked-list/description/)
* [Same Trees](https://leetcode.com/problems/same-tree/description/)
* [Sum of Left Leaves](https://leetcode.com/problems/sum-of-left-leaves/description/)
* [Valid Anagram](https://leetcode.com/problems/valid-anagram/description/)

### Day 3 - Medium

* [Most Frequent Subtree Sums](https://leetcode.com/problems/most-frequent-subtree-sum/description/)
* [Permutations](https://leetcode.com/problems/permutations/description/)
* [Rotate Image](https://leetcode.com/problems/rotate-image/description/)

### Day 4 - Easy

* [Base 7](https://leetcode.com/problems/base-7/description/)
* [Convert Sorted Array to Binary Search Tree](https://leetcode.com/problems/convert-sorted-array-to-binary-search-tree/description/)
* [Diameter of Binary Tree](https://leetcode.com/problems/diameter-of-binary-tree/description/)
* [Maximum Product of 3 Numbers](https://leetcode.com/problems/maximum-product-of-three-numbers/description/)
* [Missing Number](https://leetcode.com/problems/missing-number/description/)
* [Number of Boomerangs](https://leetcode.com/problems/number-of-boomerangs/description/)
* [Read Binary Watch](https://leetcode.com/problems/binary-watch/description/)
* [Reverse String II](https://leetcode.com/problems/reverse-string-ii/description/)
* [Student Attendance Record II](https://leetcode.com/problems/student-attendance-record-i/description/)

### Day 4 - Medium

* [Binary Tree Paths](https://leetcode.com/problems/binary-tree-paths/description/)
* [Binary Tree Right Side View](https://leetcode.com/problems/binary-tree-right-side-view/description/)
* [Clone Graph](https://leetcode.com/problems/clone-graph/description/)
* [Binary Tree to Linked List](https://leetcode.com/problems/flatten-binary-tree-to-linked-list/description/)
* [Number of Islands](https://leetcode.com/problems/number-of-islands/description/)
* [Subarray Sum equals to K](https://leetcode.com/problems/subarray-sum-equals-k/description/)
* [Subsets](https://leetcode.com/problems/subsets/description/)
* [Sum Root to Leaf Numbers](https://leetcode.com/problems/sum-root-to-leaf-numbers/description/)
* [Symmetric Tree](https://leetcode.com/problems/symmetric-tree/description/)
* [Target Sum](https://leetcode.com/problems/target-sum/description/)

### Day 4 - Hard

* [Binary Tree Maximum Path Sum](https://leetcode.com/problems/binary-tree-maximum-path-sum/description/)

### Day 5 - Easy

* [Excel Sheet Column Title](https://leetcode.com/problems/excel-sheet-column-title/description/)
* [Linked List Cycle](https://leetcode.com/problems/linked-list-cycle/description/)
* [Longest Common Prefix](https://leetcode.com/problems/longest-common-prefix/description/)
* [Merge Two Sorted Lists](https://leetcode.com/problems/merge-two-sorted-lists/description/)
* [Min Stack](https://leetcode.com/problems/min-stack/description/)
* [Panlindrome Number](https://leetcode.com/problems/palindrome-number/description/)
* [Reverse Integer](https://leetcode.com/problems/reverse-integer/description/)
* [Roman to Integer](https://leetcode.com/problems/roman-to-integer/description/)
* [Second Highest Salary](https://leetcode.com/problems/second-highest-salary/description/)
* [Sqrt(x)](https://leetcode.com/problems/sqrtx/description/)
* [Valid Parentheses](https://leetcode.com/problems/valid-parentheses/description/)

### Day 5 - Medium

* [Generate Parentheses](https://leetcode.com/problems/generate-parentheses/description/)
* [Integer to Roman](https://leetcode.com/problems/integer-to-roman/description/)
* [Kth Largest Element in an array](https://leetcode.com/problems/kth-largest-element-in-an-array/description/)
* [Letter Combinations of Phone Number](https://leetcode.com/problems/letter-combinations-of-a-phone-number/description/)
* [Longest Panlindrome Substring](https://leetcode.com/problems/longest-palindromic-substring/description/)
* [Longest Substring without Repeating Characters](https://leetcode.com/problems/longest-substring-without-repeating-characters/description/)
* [Nth Highest Salary](https://leetcode.com/problems/nth-highest-salary/description/)
* [Remove Nth Node from End of List](https://leetcode.com/problems/remove-nth-node-from-end-of-list/description/)
* [String to Integer](https://leetcode.com/problems/string-to-integer-atoi/description/)
* [Zig Zag Conversion](https://leetcode.com/problems/zigzag-conversion/description/)

### Day 6 - Easy

* [Remove duplicates from sorted array](https://leetcode.com/problems/remove-duplicates-from-sorted-array/description/) 
* [Maximum Subarray](https://leetcode.com/problems/maximum-subarray/description/)
* [Remove Element](https://leetcode.com/problems/remove-element/description/)
* [Implement strStr()](https://leetcode.com/problems/implement-strstr/description/)
* [Search Insert Position](https://leetcode.com/problems/search-insert-position/description/)
* [Count And Say](https://leetcode.com/problems/count-and-say/description/)
* [Length of Last Word](https://leetcode.com/problems/length-of-last-word/description/)
